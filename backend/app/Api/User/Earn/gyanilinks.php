...
$router->get('/api/user/earn/l4r/gyanilinks/start', function (): void {
    ...
    if ($id === 0) {
		...
		<body>
			<div class="container">
				<h1>Error</h1>
				<p>Failed to create gyanilinks code</p>
			</div>
		</body>
...
$router->get('/api/user/earn/l4r/gyanilinks/earn/(.*)', function (string $code): void {
    App::init();
    $appInstance = App::getInstance(true);
    $appInstance->allowOnlyGET();
    $session = new Session($appInstance);
    ...
    try {
        UserLock::executeWithLock($session->getInfo(UserColumns::UUID, false), function () use ($session, $coinsPerLink) {
