...
$router->get('/api/admin/roles/(.*)/permissions', function (int $roleId): void {
	...
	PermissionMiddleware::handle($appInstance, PermissionsIndex::ADMIN_PERMISSIONS_LIST);
...
$router->post('/api/admin/roles/(.*)/permissions/create', function (int $roleId): void {
	...
	$session = new MythicalDash\Chat\User\Session($appInstance);
	PermissionMiddleware::handle($appInstance, PermissionsIndex::ADMIN_PERMISSIONS_CREATE);
...
$router->post('/api/admin/permissions/update', function (): void {
	...
	$appInstance->allowOnlyPOST();
	$session = new MythicalDash\Chat\User\Session($appInstance);
	PermissionMiddleware::handle($appInstance, PermissionsIndex::ADMIN_PERMISSIONS_EDIT);
...
$router->post('/api/admin/permissions/delete', function (): void {
	...
	$appInstance->allowOnlyPOST();
	$session = new MythicalDash\Chat\User\Session($appInstance);
	PermissionMiddleware::handle($appInstance, PermissionsIndex::ADMIN_PERMISSIONS_DELETE);
	if (isset($_POST['id'])) {
		$id = $_POST['id'];
