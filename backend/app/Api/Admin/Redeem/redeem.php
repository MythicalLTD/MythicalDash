...
$router->get('/api/admin/redeem/codes', function (): void {
	...
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_REDEEM_CODES_LIST);
...
$router->post('/api/admin/redeem/code/create', function (): void {
	...
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_REDEEM_CODES_CREATE);
...
$router->post('/api/admin/redeem/code/(.*)/update', function ($codeId): void {
	...
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_REDEEM_CODES_EDIT);
	if (empty($codeId)) {
		$appInstance->BadRequest('Code ID is required', ['error_code' => 'CODE_ID_REQUIRED']);
	}
	if (!RedeemCoins::exists($codeId)) {
...
$router->post('/api/admin/redeem/code/(.*)/delete', function ($codeId): void {
	...
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_REDEEM_CODES_DELETE);
	if (empty($codeId)) {
		$appInstance->BadRequest('Code ID is required', ['error_code' => 'CODE_ID_REQUIRED']);
	}
	if (!RedeemCoins::exists($codeId)) {
