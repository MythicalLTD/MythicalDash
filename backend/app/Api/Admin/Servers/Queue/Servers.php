...
$router->get('/api/admin/server-queue', function (): void {
	...
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_SERVER_QUEUE_LIST);
...
$router->post('/api/admin/server-queue/create', function (): void {
	...
	$session = new MythicalDash\Chat\User\Session($appInstance);
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_SERVER_QUEUE_CREATE);
	if (
...
$router->get('/api/admin/server-queue/logs', function (): void {
	...
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_SERVER_QUEUE_LOGS_VIEW);
...
$router->post('/api/admin/server-queue/(.*)/delete', function (string $id): void {
	...
	PermissionMiddleware::handle($appInstance, Permissions::ADMIN_SERVER_QUEUE_DELETE);
	$serverQueueExists = ServerQueue::exists($id);
	if ($serverQueueExists) {
		global $eventManager;
