<template>
    <div class="min-h-screen bg-gray-900 flex flex-col">
        <!-- Header -->
        <GuestHeader :title="t('terms_of_service.title')" back-link="/dashboard">
            <template #metadata>
        ...
        <main class="flex-1 w-full">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div class="bg-gray-800/50 rounded-lg p-8 prose prose-invert max-w-none min-h-[300px]">
                    ...
                    <template v-else>
                        <h2 class="text-2xl font-bold text-white mb-6">{{ t('terms_of_service.title') }}</h2>
                        <div v-html="termsOfServiceHtml"></div>
                    </template>
                </div>
            </div>
        </main>
...
onMounted(async () => {
    ...
    try {
        ...
        if (data.success && data.settings && typeof data.settings.legal_tos === 'string') {
            termsOfServiceHtml.value = data.settings.legal_tos;
        } else {
