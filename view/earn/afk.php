...
if (isset($_GET['getcoins'])) {
    ...
    if ($idlecheck <= $currenttimestamp) {
        ...
        try {
            ...
            header('location: /earn/afk?e=' . $lang['login_error_unknown'] . ': <code>' . $e->getMessage() . '</code>');
            $conn->close();
            ErrorHandler::Critical('Failed to update coins ', $e);
            die ();
        }
    } else {
        header('location: /earn/afk?e=' . $lang['401_subtitle']);
        die ();
    }
} else {
    function minutesToSeconds($minutes)
...
<head>
    ...
    <title>
        <?= SettingsManager::getSetting('name') ?> - <?= $lang['afk'] ?>
    </title>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light"><?= $lang['earn'] ?> /</span> <?= $lang['afk'] ?></h4>
                            ...
                            <div class="col-md-12 grid-margin stretch-card">
                                <div class="card">
                                    <div class="card-header text-center">
                                        <div class="card-title text-center"><?= $lang['afk'] ?></div>
                                    </div>
                                    <div class="card-body text-center">
                                        <p><?= $lang['afk_text'] ?></p>
                                        <p><?= $lang['you_currently_have'] ?>
                                            <?= $session->getUserInfo('coins') ?> <?= $lang['coins'] ?>!
                                        </p>
                                        <p><?= $lang['afk_text_2']?>
                                            <?= $session->getUserInfo('minutes_afk') ?> <?= $lang['leaderboard_minutes']?>!
                                        </p>
                                        <p><?= $lang['afk_text_3']?> <span id="timer"></span>!</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        var Timer = setInterval(function () {
            if (timeleft <= 0) {
                ...
                document.getElementById("timer").innerHTML = timeleft + " <?= $lang['second']?>";
            }
            timeleft -= 1;
        }, 1000);
    </script>
