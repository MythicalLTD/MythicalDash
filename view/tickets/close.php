...
if (isset($_GET['ticketuuid']) && !$_GET['ticketuuid'] == "") {
    ...
    if (mysqli_num_rows($result) > 0) {
        $conn->query("UPDATE `mythicaldash_tickets` SET `status` = 'closed' WHERE `mythicaldash_tickets`.`ticketuuid` = '".mysqli_real_escape_string($conn,$_GET['ticketuuid'])."';");
        $conn->close();
    ...
    header('location: /help-center/tickets?e=We are missing required information');
    die();
}
?>